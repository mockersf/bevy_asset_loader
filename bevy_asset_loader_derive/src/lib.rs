...
fn impl_asset_collection(
    ...
    let bevy_ecs = manifest
        .maybe_get_path("bevy")
        .map(|bevy_path| {
            let mut segments = bevy_path.segments;
            segments.push(BevyManifest::parse_str("ecs"));
            syn::Path {
                leading_colon: None,
                segments,
            }
        })
        .or_else(|| manifest.maybe_get_path("bevy_ecs"))
        .unwrap();
    let bevy_asset = manifest
        .maybe_get_path("bevy")
        .map(|bevy_path| {
            let mut segments = bevy_path.segments;
            segments.push(BevyManifest::parse_str("asset"));
            syn::Path {
                leading_colon: None,
                segments,
            }
        })
        .or_else(|| manifest.maybe_get_path("bevy_asset"))
        .unwrap();
